...
def assign_2(values, s, d1, d2):
    """Eliminate all the other values (except d1 and d2) from values[s] and propagate.
    Return values, except return False if a contradiction is detected."""
    other_values = values[s].replace(d1, '').replace(d2, '')
    if all(eliminate(values, s, d) for d in other_values):
        return values
    else:
        return False
...
def find_naked_pairs(values, s):
    ...
    for p in peers[s]:
        if values[p] == values[s] and len(values[s]) == 2:
            ...
            assign_2(values, p, values[p][0], values[p][1])
    return values
